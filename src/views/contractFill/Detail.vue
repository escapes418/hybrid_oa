<template>
    <div v-if="isReady" class="contract_detail">
        ...
        <group :title='item.partyName'  v-for="(item, index) in dynamicFormList" :key="index" label-margin-right="1em">
            <cell-box>
                <div class="w-100">
                    <div class="detail-text">
                ...
                <div class="w-100">
                    <div class="detail-text">
                        <div class="long-content">
                            ...
                            <span class="long-detail">{{dataArr.detail.contractStartTime|stamp2TextDate}}</span>
                        </div>
                        <div class="long-content">
                        ...
                        <div class="long-content">
                            <span class="detail-title long-title">合同保管人</span>
                            <span class="long-detail">{{dataArr.detail.custodian}}</span>
                        </div>
                    </div>
                </div>
            </cell-box>
        ...
        <group title='回执情况'>
            <cell-box>
                <div class="w-100">
                    <div class="detail-text">
                        <div class="long-content">
                            <span class="detail-title long-title">回执状态</span>
                            <span class="long-detail">{{dataArr.detail.receiptStatusName}}</span>
                        </div>
                        <div class="long-content">
                            <span class="detail-title long-title">回执操作人</span>
                            <span class="long-detail">{{dataArr.detail.personName}}</span>
                        </div>
                        <div class="long-content">
                            <span class="detail-title long-title">操作人手机号</span>
                            <span class="long-detail">{{dataArr.detail.personPhone}}</span>
                        </div>
                        <div class="long-content">
                            <span class="detail-title long-title">回执操作时间</span>
                            <span class="long-detail">{{dataArr.detail.receiptTime|stamp2TextDateFull}}</span>
                        </div>
                    </div>
                </div>
            </cell-box>
        ...
        <group title='附件' label-margin-right="1em" v-for="(el, idx) in contractAttachmentList" :key="idx+10" v-if="el.upload.length!=0">
        ...
        <group title='附件' label-margin-right="1em" v-if="dataArr.detail.suppResp">
        ...
        <box gap="10px 10px">
            <template v-if="showBtn === '1'">
                <flexbox>
                    <flexbox-item>
                        <x-button type="default" :disabled="canContinue" @click.native="goContinue('2')">不再续签</x-button>
                    </flexbox-item>
                    <flexbox-item>
                        <x-button type="default" :disabled="cantContinue" @click.native="goContinue('1')">已经续签</x-button>
                    </flexbox-item>
                </flexbox>
            </template>
        </box>
            ...
            <x-dialog v-model="remarkForm.isRemarks" hide-on-blur>
                ...
                <div>
                    <div class="weui-dialog__ft">
                        <a href="javascript:;" class="weui-dialog__btn weui-dialog__btn_default" @click="remarkForm.isRemarks = false">取消</a>
    ...
    methods: {
        init() {
            ...
            if (params.id) {
                if (params.type && (params.type === 'todo' || params.type === 'done' || params.type === 'myself')) {
                    ...
                    com.covertHttp(api.contractHisDetail,{
                        id: params.id
                    }).then(rtn => {
                        if (rtn.status != 0) {
                        ...
                        this.showBtn = rtn.data.contractHisDetailResponse.button
                        ...
                        // this.dataArr.flowLoglist = rtn.data.flowLoglist || [];//流程
                        this.isReady = true;
                    })
                } else {
        ...
        agreeApply() {// 同意
            if(this.dataArr.detail.isUploadFile == 1&&this.isUploadFileCount<this.isUploadFileMin){
                this.$vux.toast.text(`缺少${this.isUploadFileMin-this.isUploadFileCount}张合同扫描件，请在电脑端上传`);
                return;
            }
            this.remarkForm.isRemarks = true;
            this.remarkForm.remarks = '';
        ...
        confirmRemark() {
            ...
            com.covertHttp(api.contractFlowNewCompleteTask,{
        ...
        del() { // 删除
            ...
            // 组件除show外的属性
                ...
                onConfirm () {
                    com.covertHttp(api.contractRepealApply,{
        ...
        cancel() { //撤销
            ...
            // 组件除show外的属性
                ...
                onConfirm () {
                    com.covertHttp(api.contractRepealTask,{
        ...
        goContinue(type){
            ...
            // 组件除show外的属性
                ...
                onCancel () {
                    ...
                    _this.cantContinue = false;
                },
                onConfirm () {
                    com.covertHttp(api.changeReceipt,{
                        contractId:_this.dataArr.detail.id,
                        receiptStatus:type
                    }).then(rtn => {
                        if (rtn.status == 0) {
                            _this.$router.push({ path: '/userIndex' });
                            com.delKeepAlive(_this,["TaskOption","ContractfillListSend","UserIndex"])
                        }
                        _this.canContinue = false;
                        _this.cantContinue = false;
                    })
        ...
        clickSupp(id){
            this.$router.push({
                path: '/contractFill/SupDetail/' + id + "/myself/1"
            });
        }
    }
}
