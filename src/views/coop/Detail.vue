<template>
  <div v-if="isReady">
            ...
            <div class="detail-text">
              ...
              <div class="long-content">
                ...
                <span class="long-detail">{{
                  dataArr.detail.initiateTime | stamp2TextDateFull
                }}</span>
              </div>
              <div class="long-content">
          ...
          <div class="w-100">
            <div class="detail-text">
              <div class="long-content">
                ...
                <span class="long-detail font-orange"
                  >{{ dataArr.detail.progressStatus
                  }}<span v-if="dataArr.detail.isEnd == 1">(结束状态)</span></span
                >
              </div>
              <div class="long-content">
                <span class="detail-title long-title">状态变更时间</span>
                <span class="long-detail">{{
                  dataArr.detail.changeTime | stamp2TextDateFull
                }}</span>
              </div>
              <div class="long-content">
              ...
              <div class="long-content">
                ...
                <span class="long-detail">{{
                  dataArr.detail.planDesignTime | stamp2TextDate
                }}</span>
              </div>
              <div class="long-content">
              ...
              <div class="long-content">
                ...
                <span class="long-detail">{{
                  dataArr.detail.planReleaseTime | stamp2TextDate
                }}</span>
              </div>
              <div class="long-content">
                <span class="detail-title long-title">实际上线时间</span>
                <span class="long-detail">{{
                  dataArr.detail.actualReleaseTime | stamp2TextDate
                }}</span>
              </div>
              <div class="long-content">
          ...
          <div class="w-100">
            <div class="detail-text">
              <div class="long-content">
                ...
                <span class="long-detail">{{ curPrincipal }}</span>
              </div>
            </div>
          </div>
        </cell-box>
          ...
          <div class="w-100">
            <div class="detail-text">
              <div class="long-content">
                ...
                <span class="long-detail">{{ participantList.join(',') }}</span>
              </div>
            </div>
          </div>
        </cell-box>
    ...
    <mulNotInput
      :showPopup="showNextPrincipal"
      @on-change="submitNextPrincipal"
      :dataList="linkmanList"
      @on-cancel="closeMulPup(1)"
    ></mulNotInput>
    <mulNotInput
      :showPopup="showEditParticipant"
      @on-change="submitParticipant"
      :dataList="linkmanList"
      :max="0"
      @on-cancel="closeMulPup(2)"
      :sendData="addPeopleList"
      :selectedItem.sync="addPeopleList"
    ></mulNotInput>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if (params.id) {
        com
          .covertHttp(api.queryNeedDetail, {
            id: params.id
          })
          .then(rtn => {
            if (rtn.status != 0) {
            ...
            this.participantList = rtn.data.participantList;
            this.curPrincipal = rtn.data.curPrincipal;
            if (
              this.dataArr.detail.operationList.length == 1 &&
              this.dataArr.detail.operationList[0] == 1
            ) {
    ...
    getCommentList(opts) {
      ...
      com
        .covertHttp(api.queryCommentList, {
          ...postData,
    ...
    getCurParticipantList() {
      this.addPeopleList = [];
      com
        .covertHttp(api.queryCurParticipantList, {
          needFlowId: this.$route.params.id
        })
        .then(rtn => {
          rtn.data.forEach((item, index) => {
    ...
    addProgress() {
      this.$router.push({
        path: `/coop/schedule/${this.dataArr.detail.typeId}/${this.$route.params.id}`
      });
    },
    submitNextPrincipal(el) {
      ...
      com
        .covertHttp(api.setNextPrincipal, {
          needFlowId: this.$route.params.id,
          userId: el[0]
        })
        .then(rtn => {
    ...
    submitParticipant(el) {
      ...
      com
        .covertHttp(api.editParticipantList, {
          needFlowId: this.$route.params.id,
          userIdList: userList
        })
        .then(rtn => {
