<template>
    <div v-if="isReady" class="rec">
        <group title='接待申请' label-margin-right="1em">
            ...
            <datetime format="YYYY-MM-DD" class="data-time" v-model="applyTime" title="申请时间" readonly placeholder="请选择申请时间"></datetime>
            ...
            <proSelector sTitle='项目名称' :isRequired="true" sPlaceholder='请选择' :sendData="approvalForm.projectId" :selectedItem.sync="approvalForm.projectId" @on-change="changeName"></proSelector>
            <x-input title="项目负责人" v-model="approvalForm.projectPersonel" readonly></x-input>
            <x-input title="预计接待人数" :max="50" type="number" v-model.trim="approvalForm.recpNum" placeholder='请输入接待人数'>
                <span slot="label">预计接待人数<span class="required">*</span></span>
            </x-input>
            <datetime class="data-time" v-model="approvalForm.recpTime" title="预计接待时间" placeholder="请选择申请时间" format="YYYY-MM-DD"></datetime>
...
export default {
    ...
    methods: {
        ...
        draft(){
            ...
            com.covertHttp(api.saveRecpFlowInfo, {
                ...postData,
                id: id
            }).then(rtn => {
                if (rtn.status == 0) {
