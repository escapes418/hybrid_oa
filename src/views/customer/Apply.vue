  ...
  <div class="approval_apply">
    <group title="基本信息" label-margin-right="1em">
      ...
      <x-input
        ...
        :max="50"
        placeholder="请输入"
        v-model.trim="applyData.custAbbreviation"
      ></x-input>
      ...
      <x-input
        ...
        :max="200"
        placeholder="请输入"
        v-model.trim="applyData.custSource"
      ></x-input>
      <x-address
        class="reddot"
        title="地址"
    ...
    <group title="业务情况" label-margin-right="1em">
      <sinSelector
        ...
        :sendData="applyData.custTrades"
        :selectedItem.sync="applyData.custTrades"
      ></sinSelector>
      <sinSelector
        sTitle="业务类型"
        sPlaceholder="请选择"
        :dataList="dictionary.custBusinessType"
        :sendData="applyData.custBusinessType"
        :selectedItem.sync="applyData.custBusinessType"
      ></sinSelector>
      <x-input
        title="规模(万元/月)"
        :max="50"
        placeholder="请输入"
        type="number"
        v-model.number="applyData.custCompanySize"
      ></x-input>
      <sinSelector
        sTitle="运力组织方式"
        sPlaceholder="请选择"
        :dataList="dictionary.custPowerMode"
        :sendData="applyData.custPowerMode"
        :selectedItem.sync="applyData.custPowerMode"
      ></sinSelector>
      <sinSelector
        sTitle="发货方式"
        sPlaceholder="请选择"
        :dataList="dictionary.custDeliverMode"
        :sendData="applyData.custDeliverMode"
        :selectedItem.sync="applyData.custDeliverMode"
      ></sinSelector>
      <sinSelector
        sTitle="收货方式"
        sPlaceholder="请选择"
        :dataList="dictionary.custReceiveMode"
        :sendData="applyData.custReceiveMode"
        :selectedItem.sync="applyData.custReceiveMode"
      ></sinSelector>
      <sinSelector
        sTitle="结算对象"
        sPlaceholder="请选择"
        :dataList="dictionary.custBalanceObj"
        :sendData="applyData.custBalanceObj"
        :selectedItem.sync="applyData.custBalanceObj"
      ></sinSelector>
      <x-input
        title="结算周期"
        :max="50"
        type="number"
        placeholder="请输入"
        v-model.trim="applyData.custBalanceCycle"
      ></x-input>
    </group>
    ...
    <group title="合同信息" label-margin-right="1em">
      <x-input
        ...
        :max="50"
        placeholder="请输入"
        v-model.trim="contractInfoSaveReq.creditCode"
      ></x-input>
      <x-input
        title="法定代表人"
        :max="50"
        placeholder="请输入"
        v-model.trim="contractInfoSaveReq.legalRepresentative"
      ></x-input>
      <x-input
        title="注册地址"
        :max="50"
        placeholder="请输入"
        v-model.trim="contractInfoSaveReq.registeredAddress"
      ></x-input>
      <x-input
        title="调度费比例(%)"
        :max="50"
        placeholder="请输入"
        type="number"
        v-model.trim="contractInfoSaveReq.dispatchProportion"
      ></x-input>
    </group>
    <group title="贸易结构" label-margin-right="1em">
      <x-textarea
        ...
        placeholder="请输入说明"
        :max="200"
        v-model="applyData.custTradeStructureReq.upstream"
      ></x-textarea>
      <x-textarea
        title="下游"
        placeholder="请输入说明"
        :max="200"
        v-model="applyData.custTradeStructureReq.downstream"
      ></x-textarea>
    </group>
    <group
      title="联系人信息"
      label-margin-right="1em"
      v-for="(item, index) in custLinkman"
      :key="index"
    >
    ...
    <box gap="10px 10px">
      <flexbox>
        <flexbox-item>
          <x-button type="primary" :disabled="isSubmit" class="mt-15" @click.native="openSubmit"
            >保存</x-button
          >
        </flexbox-item>
      </flexbox>
...
export default {
  ...
  methods: {
    ...
    openSubmit() {
      this.custLinkman.forEach((item, index) => {
    ...
    validLinkman() {
      ...
      for (let i in this.custLinkman) {
        ...
        if (!/^1[3456789][0-9]{9}$/.test(this.custLinkman[i].linkmanPhone)) {
