<template>
    <div>
        <div class="cusList">
            <group title='信息管理列表'>
                <cell-box v-for="(el,index)  in dataList" :key="index" @click.native="gotoDetail(el.id)" class="maintainbox">
                    <div>
                        <div class="disc-text custList">
                            ...
                            <div class="long-content" v-if="el.custCompanySize&&(listType==1||listType==2||listType==3)">
                            ...
                            <div class="long-content" v-if="listType==1||listType==2||listType==3">
                                <span class="disc-title long-title">客户分级</span>
                                <span class="long-text">{{el.custStageName}}</span>
                            </div>
                            <div class="long-content" v-if="listType==3">
                                <span class="disc-title long-title">跟进人</span>
                                <span class="long-text">{{el.man}}</span>
                            </div>
                            <div class="long-content" v-if="listType==1||listType==2">
                                <span class="disc-title long-title">联系人职位</span>
                                <span class="long-text">{{el.linkManPost}}</span>
                            </div>
                            <div class="long-content" v-if="listType==1||listType==4||listType==5">
                                <span class="disc-title long-title">行业</span>
                                <span class="long-text">{{el.custTradesName}}</span>
                            </div>
                            <div class="long-content" v-if="listType==4||listType==5||listType==3">
                                <span class="disc-title long-title">更新时间</span>
                                <span class="long-text">{{el.time|stamp2TextDateFull}}</span>
                            </div>
                            <div class="long-content" v-if="listType==1||listType==2">
                                <span class="disc-title long-title">开放时间</span>
                                <span class="long-text">{{el.time|stamp2TextDateFull}}</span>
                            </div>
                        </div>
                    </div>
                    <div v-if="listType==4 || listType==5" class="maintain"><x-button mini plain  @click.native.stop="goMaintain(el.id)" type="primary">维护管理</x-button></div>
                </cell-box>
            </group>
            <div class="fixed-btn" v-if="listType==1">
                <!-- <x-button class="smallbtn" type="primary" :mini="true" @click.native="goApply">新增</x-button> -->
        ...
        <div v-transfer-dom>
            <popup v-model="showSearch" position="bottom" max-height="50%">
                <group gutter="20px" label-margin-right="1em">
                    <sinSelector sTitle='所属行业' sPlaceholder='请选择' :isRequired="true" :dataList="dictionary.custTrades" :sendData="searchOpts.custTrades" :selectedItem.sync="searchOpts.custTrades"></sinSelector>
                </group>
                <box gap="10px 10px">
                    <flexbox>
                        <flexbox-item>
    ...
    methods: {
        ...
        getList(opts) {
            var postData = {
                ...
                custListPlace: this.listType,
                pageNo:1,
                pageSize:this.pageSize,
                ...this.axiosOpts,
                ...opts
            }
            com.covertHttp(api.custInfoList,{
                ...postData,
                }).then(rtn => {
        ...
        gotoDetail(id) {
            ...
            this.$router.push({ path: `/customer/detail/${id}/${this.listType}`});
        },
        search() { // 搜索
