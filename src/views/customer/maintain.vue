<template>
    <div>
        <group title='基本信息' label-margin-right="1em">
            <datetime format="YYYY-MM-DD" readonly class="data-time" v-model="custMaintenanceDate" title="维护时间" placeholder="请选择时间"></datetime>
            <sinSelector :isRequired="true" sTitle='客户级别' sPlaceholder='请选择' :dataList="dictionary.custStage" :sendData="applyData.custStage" :selectedItem.sync="applyData.custStage"></sinSelector>
            <sinSelector :isRequired="true" sTitle='联系人' sPlaceholder='请选择' :dataList="linkmanList" :sendData="applyData.linkmanId" :selectedItem.sync="applyData.linkmanId" @on-change="changeLinkman"></sinSelector>
            <sinSelector :isRequired="true" sTitle='拜访类型' sPlaceholder='请选择' :dataList="dictionary.visitType" :sendData="applyData.visitType" :selectedItem.sync="applyData.visitType"></sinSelector>
            <!-- <x-input title="问题责任人" placeholder="请输入" v-model.trim="applyData.custPersonLiable"></x-input> -->
            <x-textarea title="维护内容" placeholder='请输入说明' :max="1000" :rows="3" v-model.trim="applyData.custMaintenanceContent">
                <span slot="label" style="padding-right:20px;">维护内容<span class="required">*</span></span>
            </x-textarea>
            <!-- <x-textarea title="主要问题" placeholder='请输入说明' v-model.trim="applyData.custMaintenanceProblem"></x-textarea> -->
            <x-textarea title="备注" placeholder='请输入说明' :max="200" :rows="3" v-model.trim="applyData.remarks"></x-textarea>
        ...
        <box gap="10px 10px">
            <flexbox>
                <flexbox-item>
                    <x-button type="primary" class="mt-15" :disabled="isSubmit" @click.native="isSubmit=true">保存</x-button>
                </flexbox-item>
            </flexbox>
        </box>
        <div v-transfer-dom>
