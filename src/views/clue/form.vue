<template>
  <div>
    <group title="基本信息" label-margin-right="1em">
      ...
      <x-input
        title="客户简称"
        v-model.trim="filter.custAbbreviation"
        :readonly="isReadonly"
        placeholder="请输入客户简称"
      ></x-input>
      ...
      <sinSelector1
        sTitle="客户分类"
        sPlaceholder="请选择"
        :dataList="custClassify"
        :sendData="filter.custClassify"
        :selectedItem.sync="filter.custClassify"
      ></sinSelector1>
      <x-input
        title="规模(万吨/月)"
        :max="50"
        type="number"
        v-model.trim="filter.custCompanySize"
        placeholder="请输入"
      ></x-input>
      <mulSelector
        v-if="isReady"
        ...
        sPlaceholder="请选择"
        :dataList="tranList"
        :sendData="filter.tranMethod"
        :selectedItem.sync="filter.tranMethod"
      ></mulSelector>
      <x-input
        title="联系人"
        :max="50"
        v-model.trim="filter.custLinkmanName"
        placeholder="请输入"
      ></x-input>
      <x-input
        title="联系人职位"
        :max="50"
        v-model.trim="filter.linkmanPost"
        placeholder="请输入"
      ></x-input>
      <x-input
        title="联系方式"
        :max="50"
        v-model.trim="filter.linkmanPhone"
        placeholder="请输入"
      ></x-input>
      ...
      <x-textarea
        ...
        :max="200"
      >
        <span slot="label" style="padding-right:32px;"
          >线索说明<span class="required">*</span></span
        >
      </x-textarea>
    </group>
    ...
    <box gap="10px 10px">
      <flexbox>
        <flexbox-item>
          <x-button type="primary" :disabled="isSubmit" @click.native="isSubmit = true"
            >保存</x-button
          >
        </flexbox-item>
      </flexbox>
  ...
  methods: {
    ...
    getDetail() {
      if (this.$route.query.key) {
        com
          .covertHttp(api.custInfoClueDetail, {
    ...
    clueFormVali(self) {
      var flag = true;
      if (
        !self.filter.custName ||
        /[`~!@#$%^&*()_+<>?:"{},.\/;'[\]]/im.test(self.filter.custName) ||
        /[·！#￥（——）：；“”‘、，|《。》？、【】[\]]/im.test(self.filter.custName)
      ) {
        this.$vux.toast.text('请正确填写客户名称！');
        flag = false;
      } else if (
        (self.filter.custCompanySize && !/(^[0-9]\d*$)/.test(self.filter.custCompanySize)) ||
        self.filter.custCompanySize == 0
      ) {
        this.$vux.toast.text('请正确填写运输规模！');
        flag = false;
      } else if (
        self.filter.linkmanPhone &&
        !/^1[345678][0-9]{9}$/.test(self.filter.linkmanPhone)
      ) {
