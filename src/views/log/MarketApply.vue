<template>
    <div v-if="isReady">
        <group title='基本信息' label-margin-right="1em"    >
            ...
            <x-input title="当日签约数量" :max="50" placeholder='请输入' type="number" v-model.trim="applyData.custSignCount">
                <span slot="label">当日签约数量<span class="required">*</span></span>
            </x-input>
            <x-input title="明日拜访数量" :max="50" placeholder='请输入' type="number" v-model.trim="applyData.afterVisitCount">
                <span slot="label">明日拜访数量<span class="required">*</span></span>
            </x-input>
            <x-textarea title="明日工作计划" placeholder='请输入' :autosize="true" v-model.trim="applyData.afterDailyPlan" :max="1000">
                <span slot="label" style="padding-right:10px;">明日工作计划<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="备注" :autosize="true" v-model.trim="applyData.remarks" :max="1000"></x-textarea>
        </group>
        ...
        <group title='客户情况' label-margin-right="1em" v-for="(item, index) in applyData.dailyCustMaintenanceList" :key="index">
            ...
            <RemoteSearch sTitle='客户名称' :isRequired="true" :sLabel="item.custLabel" :itemIndex="index" sPlaceholder='请选择' @on-change="changeCust" :rOption="custOption" :sendData="item.custId" :selectedItem.sync="item.custId"></RemoteSearch>
        ...
        <group title='发送对象' label-margin-right="1em" style="margin-bottom:62px">
            <mulSelectors sTitle='发给谁' :max="0" :isRequired="true" sPlaceholder='请选择' :dataList="sendPeopelList" :sendData="sendToList" :selectedItem.sync="sendToList"></mulSelectors>
            <mulSelectors sTitle='抄送对象' :max="0" sPlaceholder='请选择' :dataList="copyToPeoList" :sendData="copyToList" :selectedItem.sync="copyToList"></mulSelectors>
        </group>
        <box class="fixbtn">
            <flexbox>
                <flexbox-item>
...
export default {
    ...
    methods: {
        ...
        changeCust(e) {
            ...
            this.applyData.dailyCustMaintenanceList[e.itemIndex].custMaintenanceContent = e.custMaintenanceContent;
            this.applyData.dailyCustMaintenanceList[e.itemIndex].custMaintenanceDate = e.custMaintenanceDate;
            this.applyData.dailyCustMaintenanceList[e.itemIndex].custMaintenanceDateTime = com.formatFullDateTime(e.custMaintenanceDate);
