<template>
    <div v-if="isReady" class="approval_detail">
        ...
        <box class="fixbtn">
            <flexbox>
                <flexbox-item>
                    <x-button type="primary" :disabled="comment.isComment" @click.native="comment.isComment = true">新增批阅</x-button>
                </flexbox-item>
            </flexbox>
        </box>
        <div v-transfer-dom>
            <x-dialog v-model="comment.isComment" hide-on-blur>
                ...
                <group class="xdialog-group-textarea">
                    <x-textarea v-model="comment.content" placeholder="请输入批阅" :max="500" :rows="3"></x-textarea>
                </group>
                <div>
                    <div class="weui-dialog__ft">
                        <a href="javascript:;" class="weui-dialog__btn weui-dialog__btn_default" @click="comment.isComment = false">取消</a>
                        <a href="javascript:;" class="weui-dialog__btn weui-dialog__btn_primary" @click="confirmComment">确定</a>
                    </div>
                </div>
            </x-dialog>
        </div>
...
export default {
    ...
    methods: {
        ...
        getCommentList(){
            ...
            com.covertHttp(api.queryDailyComment, {
                ...postData,
                id: this.$route.params.id,
            }).then(rtn => {
        ...
        confirmComment(){
            ...
            com.covertHttp(api.saveDailyComment, {
