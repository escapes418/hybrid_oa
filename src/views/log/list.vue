<template>
  <div>
    <scrollPosition>
      ...
      <div class="fixed-btn">
        <x-button type="primary" class="smallbtn" :mini="true" @click.native="showApply = true"
          >新建</x-button
        >
        <x-button type="primary" class="smallbtn" :mini="true" @click.native="showSearchBox"
          >筛选</x-button
        >
      </div>
      ...
      <popup v-model="showSearch" position="bottom" max-height="70%">
        <group gutter="20px" label-margin-right="1em">
          ...
          <sinSelector
            ...
            v-if="tabIndex == 1"
            sPlaceholder="请选择"
            :dataList="dictionary.readStatus"
            :sendData="searchOpts.readStatus"
            :selectedItem.sync="searchOpts.readStatus"
          ></sinSelector>
        </group>
        <box gap="10px 10px">
          <flexbox>
...
export default {
  ...
  methods: {
    ...
    search() {
      // 搜索
      if (
        this.searchOpts.endTime != '' &&
        new Date(this.searchOpts.endTime) < new Date(this.searchOpts.startTime)
      ) {
