<template>
    <div v-if="isReady">
        <group title='基本信息' label-margin-right="1em">
            ...
            <x-textarea title="运力池建设" :autosize="true" placeholder="请输入" v-model.trim="applyData.transportPoolBuild" :max="1000">
                <span slot="label" style="padding-right:10px;">运力池建设<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="回访项目情况" :autosize="true" placeholder="请输入" v-model.trim="applyData.revisitProjectStatus" :max="1000">
                <span slot="label" style="padding-right:10px;">回访项目情况<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="需要协助问题" :autosize="true" placeholder="请输入" v-model.trim="applyData.needAssistProblem" :max="1000">
                <span slot="label" style="padding-right:10px;">需要协助问题<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="今日感想" :autosize="true" placeholder="请输入" v-model.trim="applyData.todayThought" :max="1000">
                <span slot="label" style="padding-right:10px;">今日感想<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="备注" :autosize="true" placeholder="请输入" v-model.trim="applyData.remarks" :max="1000"></x-textarea>
        </group>
        <group title='项目情况' label-margin-right="1em" v-for="(item, index) in applyData.projectImplementStatusList" :key="index">
            ...
            <x-textarea title="具体异常说明" v-if="item.hasAbnormalStatus == 1" :autosize="true" v-model="item.anomalyDescription" :max="200">
