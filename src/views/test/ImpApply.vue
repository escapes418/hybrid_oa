<template>
    <div>
        <group title='基本信息' label-margin-right="1em"    >
            <x-textarea title="今日工作" :autosize="true" placeholder="请输入" v-model.trim="applyData.todayWork">
                <span slot="label" style="padding-right:10px;">今日工作<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="运力池建设" :autosize="true" placeholder="请输入" v-model.trim="applyData.transportPoolBuild">
                <span slot="label" style="padding-right:10px;">运力池建设<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="回访项目情况" :autosize="true" placeholder="请输入" v-model.trim="applyData.revisitProjectStatus">
                <span slot="label" style="padding-right:10px;">回访项目情况<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="需要协助问题" :autosize="true" placeholder="请输入" v-model.trim="applyData.needAssistProblem">
                <span slot="label" style="padding-right:10px;">需要协助问题<span class="required">*</span></span>
            </x-textarea>
            <x-textarea title="今日感想" :autosize="true" placeholder="请输入" v-model.trim="applyData.todayThought">
            ...
            <x-textarea title="备注" :autosize="true" placeholder="请输入" v-model.trim="applyData.remarks"></x-textarea>
        ...
        <group title='项目情况' label-margin-right="1em" v-for="(item, index) in applyData.projectImplementStatusList" :key="index">
            <box gap="10px 15px">
                <div style="height:26px">
                    <span class="fr font-orange" @click="delItem(index)">删除</span>
                </div>
            </box>
            <proSelector sTitle='项目名称' sPlaceholder='请选择' :isRequired="true" @on-change="changePro(index)" :sendData="item.projectId" :selectedItem.sync="item.projectId"></proSelector>
            <sinSelector sTitle='节点' :isRequired="true" sPlaceholder='请选择' @on-change="changeNode(index)" :dataList="item.nodeList" :sendData="item.nodeId" :selectedItem.sync="item.nodeId"></sinSelector>
            ...
            <sinSelector sTitle='异常情况' :isRequired="true" sPlaceholder='请选择' :dataList="dictionary.abnormalStatus" :sendData="item.hasAbnormalStatus" :selectedItem.sync="item.hasAbnormalStatus"></sinSelector>
            <x-textarea title="具体异常说明" v-if="item.hasAbnormalStatus == 1" :autosize="true" v-model="item.anomalyDescription">
        ...
        <group title='发送对象' label-margin-right="1em" style="margin-bottom:62px">
            <mulSelectors sTitle='发送对象' :max="0" :isRequired="true" sPlaceholder='请选择' :dataList="sendPeopelList" :sendData="sendToUserList" :selectedItem.sync="sendToUserList"></mulSelectors>
        </group>
        <box class="fixbtn">
            <flexbox>
                <flexbox-item>
...
export default {
    ...
    methods: {
        ...
        handleApply(){
            com.covertHttp(api.saveImplyDaily,{
                ...this.applyData,
