var deviceInfo = navigator.userAgent.toLowerCase(), device;
...
var sdk = {
    sendCommand: function (cmd, id, params) {
        if (device == 'iphone') {
            ...
            console.log("android.sendCommond")
    ...
    globalRecive: function (id, content) {
        console.log("globalRecive")
        var rtn={}, __flag__=false;
        // console.log(content.replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/\t/g,"\\t"),"请求数据解析----------")
        try {
            // rtn = JSON.parse(content);
            // content = content.replace(/\\/g, '\\\\');
            // content= content.replace(/\"/g, '\\"');
            // rtn = JSON.parse(content.replace(/\n/g,"\\n").replace(/\r/g,"\\r").replace(/\t/g,"\\t").replace(/\u0000/g,"\\u0000")); // replace解决字符串中有换行符报错的问题
            ...
            // rtn = JSON.parse(content.replace(/\"/g, '\\"'));
