    ...
    <div>
        ...
        <group title="个人办公" class="index-cell" v-if="myOffice == 1">
            <grid :cols="3" class="mt-15 bg-white">
                ...
                <grid-item :label="('客户信息管理')" @on-item-click="gotoCustomer" v-if="permission['inforManage-customerList'] == 1">
                ...
                <grid-item :label="('合同管理')" @on-item-click="gotoContractApply" v-if="permission['me-contractCheckList'] == 1">
        ...
        <group title="我的任务" class="index-cell" v-if="myTask == 1">
            <grid :cols="3" class="mt-15 bg-white">
                <grid-item class="long-content" :label="('待我审批')" @on-item-click="gotoApprovalListRecived" v-if="permission['listRecived'] == 1">
                    <badge v-if="todoCount != 0" class="badge" :text="todoCount"></badge>
                    <img slot="icon" src="../../assets/img/listRecived.png">
                </grid-item>
                <grid-item class="long-content" :label="('我的申请')" @on-item-click="gotoApprovalListSend" v-if="permission['listSend'] == 1">
        ...
        <group title="常用工具" class="index-cell" v-if="myTools == 1">
            <grid :cols="3" class="mt-15 bg-white">
                <grid-item class="long-content" :label="('贸易计算器')" @on-item-click="gotoCalcu" v-if="listTools['tool-colcu'] == 1">
                    <img slot="icon" src="../../assets/img/listRecived.png">
                </grid-item>
            </grid>
        </group>
...
import { Group, Flexbox, FlexboxItem, Loading, Cell, Confirm, CellBox, Grid, Badge, GridItem, GroupTitle, XButton, TransferDomDirective as TransferDom } from 'vux';

export default {
    name: "UserIndex",
    directives: { TransferDom },
    components: { Grid, GridItem, GroupTitle, Group, Loading, Cell, CellBox, Badge, Confirm, Flexbox, FlexboxItem, XButton },
    data() {
        return {
            baseInfo: {
                userName: '',
    ...
    methods: {
        ...
        sendPhoneInfo(){
            sdk.ability.fetchDeviceCode({
                success: function(res) {
                    ...
                    com.covertHttp(api.getLoginInfo,{
                        appVersion: res.data.appVersion,
                        phoneType: res.data.phoneType,
                        deviceCode: res.data.deviceCode,
                    }).then(rtn => {
        ...
        getRedCount(){
            com.covertHttp(api.findRedCount,{ //消息
                readStatus: 0,
            },false).then(rtn => {
