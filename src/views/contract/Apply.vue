<template>
    <div class="dynammic">
        <group title='基本信息' label-margin-right="1em">
            <sinSelector sTitle='合同名称' :isRequired="true" sPlaceholder='请选择' @on-change="getTemplate" :dataList="mainContractList" :sendData="applyData.contractNameId" :selectedItem.sync="applyData.contractNameId" :sLabel="contractLabel"></sinSelector>
            <PageSelector v-if="associationMain==1" :isRequired="true" sTitle='关联主合同' sPlaceholder='请选择' @selectedItem="assMainContract" :sLabel="applyData.associationMainName"></PageSelector>
            <mulProSelet v-if="associationMain!=1" sTitle='关联项目' :isRequired="true" sPlaceholder='请选择' :sendData="applyData.projectIds" :selectedItem.sync="applyData.projectIds"></mulProSelet>
            <x-textarea title="备注" v-model.trim="applyData.remarks" placeholder="请输入" :max="200" :rows="3"></x-textarea>
        </group>
        <!-- <div>{{dynamicFormList}}</div> -->
        <group :title='item.partyName' label-margin-right="1em"  v-for="(item, index) in dynamicFormList" :key="index">
            <DynamicForm :item="el" ref="dynamic"  v-for="(el, idx) in item.contractPartyType" :key="idx" @on-change="changeSele">
            </DynamicForm>
        </group> 
        <group title='合同情况' label-margin-right="1em">
            <datetime title="合同开始日期" placeholder="请选择开始日期" format="YYYY-MM-DD" v-model="applyData.contractStartTime" class="data-time">
                <span slot="title">合同开始日期<span class="required">*</span></span>
            </datetime>
            <datetime title="合同结束日期" placeholder="请选择结束日期" format="YYYY-MM-DD" v-model="applyData.contractEndTime" class="data-time">
        ...
        <group title='快递信息' label-margin-right="1em">
            <x-input title="快递公司" placeholder='请输入' :max="50" v-model.trim="applyData.expressCompany">
                <span slot="label">快递公司<span class="required">*</span></span>
            </x-input>
            <x-input title="快递单号" placeholder='请输入' :max="50" type="number" v-model.trim="applyData.expressBill">
                <span slot="label">快递单号<span class="required">*</span></span>
            </x-input>
        </group>  
        <group title='预签合同资料' label-margin-right="1em" v-for="(el, idx) in contractAttachmentList" :key="idx+10">
            <cell :title="el.attachmentName" :inline-desc="el.message+'(至少'+el.mustCount+'张)'"></cell>
            <div class="flex_box" v-if="el.upload.length!=0">
                <div class="flex_item" v-for="(val,index) in el.upload" :key="index">
                    <div class="imgbox">
                        <img src="../../assets/img/close.png" v-if="val.url" class="close" @click.stop="delImg(idx,index)">
    ...
    methods: {
        init() {
            ...
            if (this.$route.params.id != 0 ){
                this.getDetail().then(rtn=>{//1. 加载详情
                    ...
                    this.getTemplate().then(()=>{ //2. 获取表单配置信息
        ...
        assignUpload(getD){
            ...
            getD.contractAttachmentList.forEach((element,index)=>{
                for(let idx in this.contractAttachmentList){
                    if(element.fileType == this.upType && this.contractAttachmentList[idx].attachmentType == this.upType){
        ...
        getDetail() {
            return new Promise((resolve, reject) => {
                com.covertHttp(api.contractFlowDetail, {
        ...
        getSeleList(url,type){
            return new Promise((resolve, reject) => {
                com.covertHttp(baseApi.baseUrl+"/"+url, {
                    memberType:type,
                }).then(rtn => {
                    resolve(rtn.data);
                })
        ...
        getTemplate(){
            return new Promise((resolve, reject) => {
                ...
                var _this = this;
                com.covertHttp(api.queryContractConfig, {
                    ...
                    if (!rtn.data.contractConfigAttachmentList || rtn.data.contractConfigAttachmentList.length == 0) {
                        this.$vux.toast.text("合同模版加载失败");
                        return;
                    }
                    ...
                    this.dynamicFormList.forEach((element,index)=>{
                        element.contractPartyType.forEach((el,idx)=>{
                            ...
                            if(el.columnType == "select"){
                                ...
                                if(el.url != ""){
                                    _this.getSeleList(el.url,el.memberType).then(rtn=>{//获取select的选项
                                        el.DataList = rtn
                                    })
                                }
                    ...
                    rtn.data.contractConfigAttachmentList.forEach((element,idx)=>{
                        var attachmentName = ""
                        var upload = [];
                        for(let i=0;i<parseInt(element.maxCount);i++){//上传图片信息
                            upload.push({
                        ...
                        if(element.attachmentType == "1"){//合同标题
                            attachmentName = "预签合同";
        ...
        handleApply(){
            ...
            for(let idx in fields){
                for(let i in fields[idx].contractPartyType){
                    params[fields[idx].contractPartyType[i].columnName] = fields[idx].contractPartyType[i].value
                }
            }
            var attach = com.clone(this.contractAttachmentList);
            var uploadArr = [];
            attach.forEach((element,index)=>{ //去掉数组中没有上传的图片的内容//去掉没有上传图片的
        ...
        goApply(params) {  
            var _this = this;
            com.covertHttp(api.contractApplyNew, {
                ...params,
                ...this.applyData,
                ...
                if (rtn.status == 0) {
                    com.delKeepAlive(_this,["TaskOption","ContractListRecived","ContractListSend","UserIndex"])
