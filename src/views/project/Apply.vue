  ...
  <div class="approval_apply">
    <group title="基本信息" label-margin-right="1em">
      ...
      <departTree
        ...
        :isPeople="true"
        :isRequired="true"
        sPlaceholder="请选择项目管理负责人"
        :dataList="projectManagerList"
        :sendData="applyData.projectManagerId"
        :selectedItem.sync="applyData.projectManagerId"
      ></departTree>
      <departTree
        sTitle="实施负责人"
        :isPeople="true"
      ...
      <!-- <departTree
        ...
        :dataList="businessAssistantList"
        :sendData="applyData.businessAssistantId"
        :selectedItem.sync="applyData.businessAssistantId"
      ></departTree>
      <departTree
        sTitle="VIP客服"
        :isPeople="true"
        sPlaceholder="请选择VIP客服"
        :dataList="vipCustomerList"
        :sendData="applyData.vipCustomerId"
        :selectedItem.sync="applyData.vipCustomerId"
      ></departTree>
      <departTree
        sTitle="清结算"
        :isPeople="true"
    ...
    <group title="一般要求" label-margin-right="1em">
      ...
      <x-input
        ...
        :max="5"
      >
        <span slot="label" class="mr-10">月开票频次<span class="required">*</span></span>
      </x-input>
      <sinSelector
        sTitle="托管"
        :isRequired="true"
  ...
  methods: {
    ...
    getTreeList() {
      com.covertHttp(api.orgAndUserInfo, { queryType: '2' }).then(rtn => {
        ...
        this.businessAssistantList = com.clone(peopelList);
        this.projectManagerList = com.clone(peopelList);
        this.vipCustomerList = com.clone(peopelList);
        this.accountLeaderList = com.clone(peopelList);
      });
    },
    init() {
    ...
    handleApply(postData) {
      var _this = this;
      com
        .covertHttp(api.saveProjectInfo, {
          ...postData
        })
        .then(rtn => {
          if (rtn.status == 0) {
