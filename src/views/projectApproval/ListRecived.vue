<template>
  <div>
    ...
    <div v-if="apprState == 0">
      ...
      <group>
        <cell-box
          v-for="(el, index) in noApprList"
          :key="index"
          @click.native="gotoApprovalDetail(el.businessId, 'todo', el.taskId)"
        >
          <div>
    ...
    <div v-if="apprState == 1">
      ...
      <group>
        <cell-box
          v-for="(el, index) in alrApprList"
          :key="index"
          @click.native="gotoApprovalDetail(el.businessId, 'done', el.taskId)"
        >
          <div class="w-100">
            <div class="content-title">
              {{ el.applyName }}
      ...
      <popup v-model="showSearch" position="bottom" max-height="50%">
        <group gutter="20px" label-margin-right="1em">
          <x-input
            title="姓名"
            :max="50"
            v-model.trim="searchOpts.applyName"
  ...
  methods: {
    ...
    getnoApprList(opts) {
      var postData = {
        ...
        billType: 11, //billType (string, optional): 单据类型:1:市场,2:实施,3:报销,4:接待申请 ,
        ...this.axiosOpts,
        ...opts
      };
      com
        .covertHttp(api.getNoApprList, {
          ...postData,
    ...
    gotoApprovalDetail(businessId, type, taskId) {
      ...
      if (taskId == '') return;
      this.$router.push({
        path: '/projectApproval/detail/' + businessId + '/' + type + '/' + taskId
      });
    },
    clickLoadMore(opts) {
      this.page++;
