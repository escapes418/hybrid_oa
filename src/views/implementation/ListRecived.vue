    ...
    <div>
        ...
        <div v-if="apprState==0">
            ...
            <group>
                <cell-box v-for="(el,index)  in noApprList" :key="index" @click.native="gotoApprovalDetail(el.businessId,'todo',el.taskId)">
                    <div>
                        <div class="content-title">{{el.applyName}}
                            <span>发起的实施需求申请</span>
                        </div>
                        <div class="disc-text">
        ...
        <div v-transfer-dom>
            <popup v-model="showSearch" position="bottom" max-height="50%">
                <group gutter="20px" label-width="6em" label-margin-right="1em" label-align="justify">
                    <x-input title="姓名" v-model="searchOpts.applyName" placeholder="请输入申请人姓名"></x-input>
                     <x-input title="流程编号" v-model="searchOpts.procCode" placeholder="请输入流程编号"></x-input>
...
export default {
    ...
    methods: {
        ...
        getnoApprList(opts) {
            ...
            com.covertHttp(api.getNoApprList, postData).then(rtn => {     
        ...
        getAlrApprList(opts) {
            ...
            com.covertHttp(api.getAlrApprList,postData).then(rtn => {
        ...
        gotoApprovalDetail(businessId, type,taskId) {
            ...
            if (taskId == "") return
            this.$router.push({ path: "/implementation/detail/" + businessId + "/" + type + "/" + taskId });
        },
        clickLoadMore(opts) {
            this.page++;
            var postData = {
