  ...
  <div v-if="isReady" class="approval_detail">
    <group title="">
      <group-title slot="title"
        >报销申请
        <div style="float:right;color:#399df7;" @click="goHistoryDetail">
          历史报销记录
          <x-icon
            type="ios-arrow-right"
            size="14"
            style="fill:#399df7;margin-left:-5px;margin-top:-2px;vertical-align:middle"
          ></x-icon>
        </div>
      </group-title>
      <cell-box>
        <div class="w-100">
          <div class="detail-text">
            ...
            <div
              v-if="
                dataArr.detail.applyType == 2 &&
                  dataArr.detail.expenseStatus != '1' &&
                  dataArr.detail.expenseStatus != '0'
              "
              class="long-content"
            >
    ...
    <group title="" v-if="dataArr.flowDetailList.length != 0">
      <group-title slot="title"
        >科目费用
        <div style="float:right;color:#399df7;" @click="goDetailItem">
          查看科目明细
          <x-icon
            type="ios-arrow-right"
            size="14"
      ...
      <div class="selectbox" v-for="(el, index) in amtList" :key="index">
        <div class="stitle">
          {{ el.label }}<span class="sign" v-if="el.isExceed == 1">异常</span>
        </div>
    ...
    <group title="审批流程" v-if="dataArr.detail.expenseStatus != 4">
      <timeline class="font-gray adjustTimeline">
        <timeline-item v-for="(el, index) in dataArr.flowLoglist" :key="index">
          <div
            :class="['ml-10', index == dataArr.flowLoglist.length - 1 ? 'font-green' : 'font-g333']"
          >
            <div>
      ...
      <flexbox>
        ...
        <flexbox-item :span="2 / 5">
          <flexbox v-if="type === 'myself'">
            ...
            <flexbox-item
              v-if="dataArr.detail.expenseStatus != 1 && dataArr.detail.expenseStatus != 0"
            >
              <x-button class="bottombtn" type="default" :disabled="disDel" @click.native="del"
                >删除</x-button
              >
            </flexbox-item>
            <flexbox-item v-if="dataArr.detail.expenseStatus == 2 && !ISEDIT">
            ...
            .then(rtn => {
              ...
              if (this.dataArr.flowDetailList.length != 0) {
                rtn.data.amtList.forEach((item, index) => {
                  this.amtList.push({
                    ...item,
                    label: item.secondSubName
                      ? `${item.firstSubName}/${item.secondSubName}`
                      : item.firstSubName,
                    value: `￥${item.amt}`
                  });
                });
