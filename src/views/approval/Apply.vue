<template>
    <div class="approval_apply">
        ...
        <group title='报销申请' label-margin-right="1em">
            <!-- 基本读取信息 START -->
            <x-input title="报销人" readonly :value="baseInfo.userName" placeholder="请输入报销人姓名"></x-input>
            <datetime format="YYYY-MM-DD" class="data-time" v-model="baseInfo.applyTime" title="报销时间" placeholder="请选择开始时间" readonly></datetime>
            <x-input title="所属部门" readonly :value="baseInfo.officeName" placeholder="请选择所属部门"></x-input>
            <!-- 基本读取信息 END -->
            <sinSelector1 sTitle='报销类型' :isRequired="true" sPlaceholder='请选择' @on-change="changeType" :dataList="dictionary.oaExpenseType" :sendData="approvalForm.applyType" :selectedItem.sync="approvalForm.applyType"></sinSelector1>
            <x-input title="关联申请" v-if="approvalForm.applyType != 1" :value="relationThemeName" placeholder='请选择' @click.native="themeShow = true" readonly>
                <span slot="label">关联申请<span class="required">*</span></span>
            </x-input>
            <proSelector sTitle='关联项目' v-if="approvalForm.applyType == 1" sPlaceholder='请选择' :sendData="approvalForm.projectId" :selectedItem.sync="approvalForm.projectId"></proSelector>
            <x-input title="项目名称" v-if="approvalForm.applyType != 1" :value="approvalForm.projectLabel" readonly></x-input>
            <x-input title="报销分类" v-if="approvalForm.applyType == 3" :value="approvalForm.travelExpenseTypeListName" readonly></x-input>
            <x-input title="随行人员" v-if="approvalForm.applyType == 3" :value="approvalForm.entourageListName" readonly></x-input>
            <RelEmployees sTitle='陪客人员' :isRequired="true" v-if="approvalForm.applyType == 2" sPlaceholder='请选择' :dataList="employeesList" :sendData="approvalForm.employees" :selectedItem.sync="approvalForm.employees">
            </RelEmployees>
            <sinSelector3 sTitle='发票公司' :isRequired="true" sPlaceholder='请选择' :dataList="taxCityList" :sendData="approvalForm.taxCity" :selectedItem.sync="approvalForm.taxCity"></sinSelector3>
            <x-textarea title="接待客户情况" v-if="approvalForm.applyType == 2" v-model="approvalForm.customerSituation" placeholder="请输入接待客户情况" :max="600" :rows="3"></x-textarea>
            <x-textarea title="备注" v-model="approvalForm.remarks" placeholder="请输入备注" :max="700" :rows="3"></x-textarea>
        </group>
        <group title='报销明细' label-margin-right="1em" v-for="(item, index) in itemDatas" :key="index">
        ...
        <div v-transfer-dom>
            <confirm v-model="isSubmit" title="提交报销申请" @on-cancel="isSubmit=false" @on-confirm="submit">
            ...
            <popup v-model="themeShow" height="100%">
                <div class="popup1">
                    <group v-if="themelistData.length!=0">
                        <cell-box v-for="(el,index) in themelistData" :key="index" @click.native="themeProcCb(index)">
                            <div class="w-100">
                                <div class="disc-text">
...
import { XAddress, Box, Popup, Checklist, Search, Group, Cell, XDialog, CellBox, Icon, XButton, PopupPicker, XInput, XTextarea, Selector, Confirm, Calendar, Datetime, Flexbox, FlexboxItem, TransferDomDirective as TransferDom } from 'vux';
export default {
    name: "EditApproval",
    directives: {
        TransferDom
    ...
    components: {
        Box, Group, Cell, CellBox, XButton, Popup, PopupPicker, Checklist, Datetime, Search, XInput, XTextarea, Selector, Confirm, Icon, Calendar, Flexbox, FlexboxItem, XAddress, XDialog, ReimItem,
    ...
    methods: {
        ...
        // 获取关联主题的任务信息
        getThemeList() {
            com.covertHttp(api.flowRevencelist,{
        ...
        valid() { // 整体校验
            ...
            for (var i in _this.itemDatas) {//检验明细
        ...
        handleApply() {
            ...
            com.covertHttp(api.editApproval, {
                ...postData
            }).then(rtn => {
                if (rtn.status == 0) {
                    this.$vux.toast.text(rtn.message || "申请发起成功");
                    this.$router.push({ path: '/userIndex' });
                    com.delKeepAlive(this,["TaskOption","ApprovalListRecived","ApprovalListSend","UserIndex"])
                }
        ...
        draft(){
            ...
            com.covertHttp(api.saveExpenseInfo, {
                ...postData,
                id: id
            }).then(rtn => {
                if (rtn.status == 0) {
        ...
        themeProcCb(idx) {
            ...
            if(this.themelistData[idx].travelExpenseTypeListName){
                this.approvalForm.travelExpenseTypeListName = this.themelistData[idx].travelExpenseTypeListName.join(', ');
            }
