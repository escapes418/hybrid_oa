        ...
        <div v-transfer-dom>
            <popup v-model="showSearch" position="bottom" max-height="50%">
                <group gutter="20px" label-margin-right="1em">
                    <x-input title="姓名" v-model.trim="searchOpts.applyName" :max="50" placeholder="请输入申请人姓名"></x-input>
                    <x-input title="流程编号" v-model.trim="searchOpts.procCode" :max="50" placeholder="请输入流程编号"></x-input>
                    <x-input title="部门名称" v-model.trim="searchOpts.officeName" :max="50" placeholder="请输入部门名称"></x-input>
    ...
    methods: {
        ...
        getnoApprList(opts) {
            var postData = {
                ...
                billType: 3,//billType (string, optional): 单据类型:1:市场,2:实施,3:报销,4:接待申请 ,
                ...this.axiosOpts,
                ...opts
            }
        ...
        gotoApprovalDetail(businessId, type,taskId) {
            ...
            if (taskId == "") return
            this.$router.push({ path: "/approval/detail/" + businessId + "/" + type + "/" + taskId });
        },
        clickLoadMore(opts) {
            this.page++;
            var postData = {
