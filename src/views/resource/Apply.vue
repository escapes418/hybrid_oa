<template>
    <div class="approval_apply">
        <group title='基本信息' label-margin-right="1em">
            ...
            <proSelector sTitle='项目名称' :isRequired="true" sPlaceholder='请选择' :sendData="applyData.projectId" :selectedItem.sync="applyData.projectId" @on-change="changeName"></proSelector>
            <x-input title="项目负责人" v-model="applyData.projectPersonel" readonly></x-input>
            <sinSelector sTitle='资源类型' :isRequired="true" sPlaceholder='请选择' :dataList="resourceList" :sendData="applyData.resourcesType" :selectedItem.sync="applyData.resourcesType"></sinSelector>
            <x-input title="需求数量" :max="50" placeholder="请输入" v-model.trim="applyData.demandPersonelNum">
                <span slot="label">需求数量<span class="required">*</span></span>
            </x-input>
            <datetime title="期望抵达时间" format="YYYY-MM-DD HH" class="data-time" v-model="applyData.expectDate" placeholder="请选择时间" :display-format="expectDate"></datetime>
            ...
            <x-input title="预算金额" :max="50" placeholder="请输入" v-model.trim="applyData.amountSum" type="number">
                <span slot="label">预算金额<span class="required">*</span></span>
            </x-input>
            <x-textarea title="备注" v-model.trim="applyData.remarks" :autosize="true" :max="200"></x-textarea>
        </group>
        <box gap="10px 10px">
            <flexbox>
                <flexbox-item>
...
export default {
    ...
    methods: {
            ...
            if (this.$route.params.id) {
                this.getDetail().then(res =>{
                    if(!res.resourcesApplyFlowResponse.expectDate){
                        ...
                        //转换成合适的时间格式
                        res.resourcesApplyFlowResponse.expectDate = com.timeFormatHour(res.resourcesApplyFlowResponse.expectDate)
                    }
        ...
        handleApply(){
            com.covertHttp(api.resourcesApply,{
                ...this.applyData,
                expectDate:com.timeParse(this.applyData.expectDate+":00:00"),
                    ...
                    com.delKeepAlive(this,["TaskOption","UserIndex","ResourceListRecived","ResourceListSend"])
                }
        ...
        draft(){
            ...
            com.covertHttp(api.saveResourcesApplyInfo,{
                id: id,
                ...this.applyData,
                expectDate:com.timeParse(this.applyData.expectDate+":00:00")
            }).then(res=>{
        ...
        valid(){
            ...
            if(!this.applyData.projectId){
                ...
                flag = false;
            }else if((!/(^[0-9]\d*$)/.test(this.applyData.demandPersonelNum)) || this.applyData.demandPersonelNum == 0){
