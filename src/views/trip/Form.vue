<template>
    <div class="approval_apply">
        <group title='出差申请' label-margin-right="1em">
            ...
            <datetime format="YYYY-MM-DD" class="data-time" v-model="baseInfo.currentDate" title="申请时间" placeholder="请选择时间" readonly></datetime>
            <x-input title="所属部门" :value="baseInfo.officeName" placeholder="请选择所属部门" readonly></x-input>
            <relatedType sTitle='关联类型' :isRequired="true" sPlaceholder='请选择类型' :dataList="relatedTypeJSON" :sendData="approvalForm.relType" :selectedItem.sync="approvalForm.relType" @update:selectedItem='typeSlted' @on-change="changeType"></relatedType>
            <!-- 关联主题 -->
            <x-input title="关联主题" v-if="approvalForm.relType==1" :value="relationThemeName" placeholder='请选择' @click.native="openTheme = true" readonly>
                <span slot="label">关联主题<span class="required">*</span></span>
            </x-input>
            <!-- 关联项目 -->
            <div v-if="approvalForm.relType==2">
                <proSelector :isRequired="true" sTitle='关联项目' sPlaceholder='请选择项目' :sendData="approvalForm.projectId" :selectedItem.sync="approvalForm.projectId" @on-change="projectSlted(approvalForm.projectId)"></proSelector>
            </div>
            <x-input title="项目名称" v-if="approvalForm.relType==1" :value="procName" placeholder="" readonly></x-input>
            <x-input title="项目负责人" v-if="approvalForm.relType==1||approvalForm.relType==2" :value="projectLeaderName" placeholder="" readonly></x-input>
            <mulSelectors sTitle='报销分类' :isRequired="true" :max="0" sPlaceholder='请选择' :dataList="dictionary.travelExpenseType" :sendData="approvalForm.travelExpenseTypeList" :selectedItem.sync="approvalForm.travelExpenseTypeList"></mulSelectors>
            ...
            <x-textarea title="备注" v-model="approvalForm.remarks" placeholder="请输入备注" :max="200" :rows="3"></x-textarea>
        </group>
...
export default {
    ...
    methods: {
        ...
        getThemeList(){
            //获取关联主题
            com.covertHttp(api.resourcesRelationHandleFlowList,{
        ...
        handleApply() {
            ...
            com.covertHttp(api.travelApply, {
                ...postData
            }).then(rtn => {
                if (rtn.status == 0) {
        ...
        draft(){
            ...
            com.covertHttp(api.saveTravelFlowInfo, {
                ...postData,
                id: id
            }).then(rtn => {
                if (rtn.status == 0) {
