<template>
    <div class="approval">
        <group label-margin-right="1em">
            ...
            <div class="flex_box" v-if="itemData.subConfList.length!=0">
                <div class="flex_item" v-for="(val,index) in itemData.subConfList" :key="index">
                    <div class="imgbox">
                        <img src="../assets/img/close.png" v-if="val.url" class="close"  @click.stop="delImg(index)">
            ...
            <x-input v-model.number="itemData.billNum" title="票据数量" placeholder="请填写票据数量" type="number">
        ...
        <popup-picker ref='subjectPicker' :show.sync="showSubject" v-model="itemData.subject" :data="subjectData" :columns="2" style="display:none;" @on-hide="hideCb"></popup-picker>
    </div>
</template>
<script>
        ...
        hideCb(closeType) {
            if (closeType) {
                ...
                if (secondId.indexOf('no') == -1) {
                    ...
                    newObj.subConfList.forEach((el,idx)=>{//图片提示文字超过7个字就截掉
